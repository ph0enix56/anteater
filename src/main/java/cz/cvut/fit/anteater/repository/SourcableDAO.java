...
public class SourcableDAO<T extends SourceableEntity> {

	private final MongoTemplate mongoTemplate;
	@NonNull private final Class<T> entityClass;

	public SourcableDAO(MongoTemplate mongoTemplate, @NonNull Class<T> entityClass) {
	...
	public Page<T> search(String name, List<Source> sources, @NonNull Pageable pageable) {
		Query query = new Query();
		if (name != null) query.addCriteria(Criteria.where("name").regex(name, "i"));
		if (sources != null) query.addCriteria(Criteria.where("source").in(sources));
